...
- **A book can be prepared while you are doing something else.** The run belongs to the app, not to the sheet that started it: close the sheet, read another book, switch tabs - it keeps going, and a bar above the mini player shows which book, how far along, and how much time is left, with a stop control. Start "Prepare whole document" and the app also holds an audio session (a looping 60 Hz tone at -66 dBFS - inaudible to a person, but a real signal, because a browser decides a tab is playing audio from the power of its output and a track of digital zeros earns none of the exemptions it is played for) plus a screen wake lock, so putting the phone down, locking it, or switching apps does not suspend the work - the same mechanism that keeps music playing when you leave a music app. The media notification carries the same progress and a stop button, because a silent session a user cannot see or cancel is indistinguishable from an app quietly eating their battery. The loop yields through a `MessageChannel` rather than `setTimeout`, since a hidden page has its timers throttled to once a second or once a minute. Cancellation is checked between sections, so stopping waits for the section in flight - the bar says so rather than appearing to ignore the press. **Closing the app entirely stops it** - no service worker can host a neural network for half an hour - but nothing is lost: chunks are cached individually, so reopening and pressing prepare again skips everything already done. Time remaining is measured from the rate the run is achieving, never predicted, and if nothing completes for four minutes the bar says so - an OS that suspends the app anyway is a thing a reader needs told, not hidden from.
