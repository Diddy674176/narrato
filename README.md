...
- **The backend is chosen by capability, not form factor.** An earlier rule pinned every phone to WASM based on its user agent, which treated a flagship with a desktop-class GPU exactly like a budget handset. Auto now uses **WebGPU whenever the browser exposes it and the device has ≥6 cores** — so a modern flagship gets its GPU. Phones on WebGPU take fp16 (163 MB) rather than fp32 (326 MB), since GPUs compute in fp16 natively and halving a one-time download matters more on a phone. Any initialisation failure falls back to WASM + q8, so the worst case is what the device would have had anyway. Settings → Diagnostics overrides both, and the active precision is part of the audio cache key so q8 and fp16 audio never get mixed inside one book.
...
npm run test:e2e   # 27 assertions: real browser, full import → listen flow
...
- **select** — engine selection across real device profiles: a flagship phone gets WebGPU, a low-core or WebGPU-less device stays on the safe WASM path, an unknown core count is treated conservatively, and an explicit choice is honoured unless the browser cannot provide it.
