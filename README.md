...
- **The model is warmed on later visits, not on first sight.** If the weights are already in Cache Storage the engine starts loading when the app opens, so pressing Play is instant; if they are not, nothing is downloaded until you ask, because 92 MB of someone's mobile data is not ours to spend speculatively.
